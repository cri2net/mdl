	...
	<?php
		if($isErrorStep)
		{
			...
			<form class="gerts-register" name="gerts-register" method="post" action="<?= BASE_URL . $_SERVER['REQUEST_URI']; ?>" id="register">
				<div class="error" id="error_reg">
					<p>
						<span id="error_msg"><?= $error_msg; ?></span><br />
					</p>
				</div>
			</form>
			<?php
		...
		elseif ($isSuccessStep)
		{
			...
			<form class="gerts-register" name="gerts-register" method="post" action="<?= BASE_URL . $_SERVER['REQUEST_URI']; ?>" id="register">
				<div class="success">
					...
					<span><a style="font-size:14px;" onclick="sendInvoice(); return false;" href="#">Скачать квитанцию об оплате в PDF &darr;</a></span>
				</div>
			</form>
