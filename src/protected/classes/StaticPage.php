...
{   
    ...
    /**
     * Отдаёт UserAgent по его полной строке
     * Если он не найден, создаёт новую запись в БД и отдаёт её
     * 
     * @param  string $user_agent_string Значение заголовка User-Agent
     * @return array
     */
    public static function getByName($user_agent_string)
    {
        $stm = PDO_DB::prepare("SELECT * FROM " . TABLE_PREFIX . "page_views_user_agents WHERE user_agent_string=? LIMIT 1", [$user_agent_string]);
        $row = $stm->fetch();

        if ($row === false) {
            $id = self::addUserAgent($user_agent_string);
            return self::getByName($user_agent_string);
        }
